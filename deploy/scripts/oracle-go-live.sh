...
# Usage:
#   DOMAIN=seodeva.com bash deploy/scripts/oracle-go-live.sh
# Optional:
...
if [[ -z "${DOMAIN}" ]]; then
  echo "ERROR: DOMAIN is required. Example: DOMAIN=seodeva.com bash deploy/scripts/oracle-go-live.sh"
  exit 1
fi
