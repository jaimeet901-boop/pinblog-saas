...
import {
	Save, RotateCcw, Upload, Download,
} from 'lucide-react';
import { AdminHero, StatusPill } from '@/components/admin/AdminUi';
import { MOCK_PLATFORM_SETTINGS as INITIAL } from '@/pages/admin/platformSettingsMock';
...
export default function AdminSettingsPage() {
	...
	const reset = () => {
		...
		toast({ title: 'Settings reset', description: 'Mock configuration restored locally.' });
	};
	...
	const blockedSave = () => {
		toast({
			variant: 'destructive',
			title: 'Save disabled',
			description: 'Backend is not connected. Changes stay local in this session only.',
		});
	};
	...
	return (
		<div>
			<AdminHero
				...
				description="Configure platform-wide defaults, integrations and feature controls. Mock configuration — no persistence."
				action={(
					<div className="admin-analytics-controls">
						...
						<button type="button" className="admin-btn admin-btn--primary" disabled={!BACKEND_READY} onClick={blockedSave} title="Backend not available">
			...
			<p className="admin-note mt-0 mb-3">
				{dirty ? 'Local edits pending · not saved to server' : 'Showing mock defaults'}
				{' · '}
				All save/update actions stay disabled until Admin Console APIs exist.
			</p>
			...
			<div className="admin-settings-grid">
				<Section title="General">
					<div className="admin-config-grid">
						...
						<TextSelect
							...
							value={settings.general.defaultWorkspacePlan}
							onChange={(value) => patch('general', 'defaultWorkspacePlan', value)}
							options={['free', 'starter', 'pro', 'business', 'enterprise']}
						/>
					</div>
					<div className="mt-2 space-y-2">
						<ToggleRow label="Maintenance Mode" checked={settings.general.maintenanceMode} onChange={(value) => patch('general', 'maintenanceMode', value)} />
						<ToggleRow label="Allow Registration" checked={settings.general.allowRegistration} onChange={(value) => patch('general', 'allowRegistration', value)} />
					</div>
				</Section>
				...
				<Section title="AI Configuration">
					...
					<div className="mt-2 space-y-2">
						<ToggleRow label="Streaming Enabled" checked={settings.ai.streamingEnabled} onChange={(value) => patch('ai', 'streamingEnabled', value)} />
						<ToggleRow label="Reasoning enabled" checked={settings.ai.reasoningEnabled} onChange={(value) => patch('ai', 'reasoningEnabled', value)} />
					</div>
				</Section>
				...
				<Section title="Content Defaults">
					...
					<div className="mt-2 space-y-2">
						<ToggleRow label="SEO Enabled" checked={settings.content.seoEnabled} onChange={(value) => patch('content', 'seoEnabled', value)} />
						<ToggleRow label="Auto Meta Description" checked={settings.content.autoMetaDescription} onChange={(value) => patch('content', 'autoMetaDescription', value)} />
						<ToggleRow label="Auto Slug" checked={settings.content.autoSlug} onChange={(value) => patch('content', 'autoSlug', value)} />
						<ToggleRow label="Auto Categories" checked={settings.content.autoCategories} onChange={(value) => patch('content', 'autoCategories', value)} />
						<ToggleRow label="Auto Tags" checked={settings.content.autoTags} onChange={(value) => patch('content', 'autoTags', value)} />
						<ToggleRow label="Internal Linking" checked={settings.content.internalLinking} onChange={(value) => patch('content', 'internalLinking', value)} />
					</div>
				</Section>
				...
				<Section title="Image Settings">
					<div className="admin-config-grid">
						<TextSelect
							...
							value={settings.images.defaultImageProvider}
					...
					<div className="mt-2">
						<ToggleRow label="Watermark" checked={settings.images.watermark} onChange={(value) => patch('images', 'watermark', value)} />
					</div>
				</Section>
				...
				<Section title="WordPress Defaults">
					<div className="admin-config-grid">
						<TextSelect
							...
							value={settings.wordpress.publishingStatus}
					...
					<div className="mt-2 space-y-2">
						<ToggleRow label="Featured Image Required" checked={settings.wordpress.featuredImageRequired} onChange={(value) => patch('wordpress', 'featuredImageRequired', value)} />
						<ToggleRow label="Auto Publish" checked={settings.wordpress.autoPublish} onChange={(value) => patch('wordpress', 'autoPublish', value)} />
					</div>
				</Section>
				...
				<Section title="Pinterest Defaults">
					<div className="admin-config-grid">
						...
						<TextSelect
							label="Scheduling"
							value={settings.pinterest.scheduling}
						...
						<TextSelect
							label="Image Ratio"
							value={settings.pinterest.imageRatio}
				...
				<Section title="Email Settings">
					<div className="admin-config-grid">
						<Field label="SMTP Status">
							<div className="pt-2"><StatusPill status={settings.email.smtpStatus} /></div>
						</Field>
				...
				<Section title="Security">
					...
					<div className="mt-2">
						<ToggleRow label="Require 2FA" checked={settings.security.require2fa} onChange={(value) => patch('security', 'require2fa', value)} />
					</div>
				</Section>
				...
				<Section title="Feature Flags" hint="UI switches only — flags are not persisted.">
				...
				<Section title="License">
					<div className="admin-config-grid">
						...
						<TextSelect
							...
							value={settings.license.releaseChannel}
