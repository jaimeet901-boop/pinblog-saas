...
export default function AdminFacebookPage() {
	...
	const loadAccounts = useCallback(async () => {
		...
		try {
			...
			const response = await apiServerClient.fetch(`/admin/v1/inventory/facebook-accounts?${params.toString()}`);
			if (!response.ok) throw new Error(await readApiError(response));
			const payload = await response.json();
			setAccounts(Array.isArray(payload.items) ? payload.items : []);
		} catch (error) {
			toast({ variant: 'destructive', title: 'Facebook accounts failed', description: error.message });
		} finally {
			setLoading(false);
		}
	}, [search, status, toast]);
	...
	const loadOauth = useCallback(async () => {
		...
		try {
			const response = await apiServerClient.fetch('/admin/v1/facebook/oauth-config');
			if (!response.ok) throw new Error(await readApiError(response));
			const payload = await response.json();
			setOauth(payload);
	...
	const saveOauth = async (event) => {
		...
		try {
			...
			setOauth(payload);
			setAppSecret('');
			toast({ title: 'Facebook OAuth saved', description: 'App credentials are stored encrypted in PocketBase.' });
		} catch (error) {
	...
	return (
		<div>
			<AdminHero
				...
				description="Platform Facebook OAuth app credentials and connected account inventory from PocketBase."
			/>

			<section className="admin-card mb-4">
				<div className="flex items-start justify-between gap-3 mb-3">
					<div>
						<h3 className="m-0">Facebook OAuth App</h3>
					...
					<form className="space-y-3" onSubmit={saveOauth}>
						{oauth.trialAccessPending && (
							<p className="admin-note mt-0">
								App review / credentials pending — configure App ID and App Secret once Meta app access is ready. Workspace Connect waits until credentials are live.
							</p>
						)}
						<div className="admin-config-grid">
							...
							<label className="md:col-span-2">
								...
								<input
									...
									placeholder="https://api.example.com/facebook/oauth/callback"
									autoComplete="off"
								/>
							</label>
							<label className="md:col-span-2">
								...
								<input
									...
									placeholder={EMPTY_OAUTH.scopes}
									autoComplete="off"
								/>
							</label>
						</div>
				...
				<div className="admin-workspace-grid">
					{accounts.map((account) => (
						<article key={account.id} className="admin-workspace">
							...
							<p>Pages · {account.pages}</p>
							<p>Connected · {account.connectedAt || '—'}</p>
							<p>Token expires · {account.expiresAt || '—'}</p>
							<p>Last sync · {account.lastSyncAt || '—'}</p>
							{account.statusError ? <p className="text-red-600">Error · {account.statusError}</p> : null}
						</article>
					))}
				</div>
			)}
