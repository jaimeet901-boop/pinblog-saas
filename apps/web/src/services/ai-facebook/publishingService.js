/**
 * Facebook Publish Now + progress polling for Content Studio.
 */

import apiServerClient from '@/lib/apiServerClient';
import { formatFacebookPublishError } from './facebookErrors.js';
...
function resolvePinTarget({ pinId, accountId, boardId, perPinTargets }) {
	const target = perPinTargets?.[pinId] || {};
	return {
		accountId: target.accountId || accountId,
		pageId: target.pageId || target.boardId || boardId,
	};
}
...
export async function publishNow({
	...
	if (!Array.isArray(pinIds) || pinIds.length === 0) {
		throw new Error('Select at least one post to publish');
	}
	if (!accountId) throw new Error('Select a Facebook account');
	if (!boardId) throw new Error('Select a Facebook Page');

	const targets = mapPerPinTargets(perPinTargets);
	const jobs = [];

	for (const pinId of pinIds) {
		const target = resolvePinTarget({ pinId, accountId, boardId, perPinTargets: targets });
		const response = await apiServerClient.fetch('/facebook/publish', {
			method: 'POST',
...
export async function fetchJobsByIds(jobIds = []) {
	...
	const response = await apiServerClient.fetch('/facebook/jobs?perPage=200&page=1', { method: 'GET' });
	const body = await parseJson(response);
	if (!response.ok) {
...
export async function fetchScheduledJobs({ status = 'scheduled', perPage = 200 } = {}) {
	...
	const response = await apiServerClient.fetch(`/facebook/jobs?${params}`, { method: 'GET' });
	const body = await parseJson(response);
	if (!response.ok) {
...
export async function watchPublishProgress({
	...
	onProgress?.({
		...
		jobs: [],
		elapsedMs: 0,
		message: 'Publish jobs created — waiting for Facebook…',
	});
	...
	return {
		...
		timedOut: true,
		message: 'Timed out waiting for Facebook. Check Calendar for final status.',
	};
}
...
export function summarizePublishResult(jobs = []) {
	...
	return {
		...
		facebookResponses: jobs.map((job) => ({
			jobId: job.id,
			status: job.status,
			pinId: job.facebookPostId || '',
			pinUrl: job.facebookPostUrl || '',
			error: formatFacebookPublishError(job.lastError || ''),
...
export async function runPublishNowFlow({
	...
	onProgress?.({ phase: 'submitting', jobs: [], elapsedMs: 0, message: 'Submitting to Facebook…' });
