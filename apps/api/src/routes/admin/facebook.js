...
import {
	ensureFacebookAppCredentialsSeeded,
	getFacebookAppCredentialsPublic,
	upsertFacebookAppCredentials,
} from '../../services/facebook/app-credentials.js';
...
router.get('/oauth-config', asyncHandler(async (_req, res) => {
	await ensureFacebookAppCredentialsSeeded().catch(() => null);
	const config = await getFacebookAppCredentialsPublic();
	res.json(config);
}));
...
router.put('/oauth-config', asyncHandler(async (req, res) => {
	...
	const config = await upsertFacebookAppCredentials({
		appId,
