...
export async function enqueueWordpressPublish(ownerOrCtx, payload = {}) {
	...
	if (clientKey) {
		try {
			const existing = await findPublishJobByOwnerIdempotencyKey(ownerId, idempotencyKey, pocketbaseClient);
	...
	const job = await createPublishJobIdempotent(ownerId, idempotencyKey, createBody, pocketbaseClient);
