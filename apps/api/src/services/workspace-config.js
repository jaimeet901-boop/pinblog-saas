...
export async function buildWorkspaceConfig(req) {
	...
	const payload = stripSecrets({
		...
		general: withProvenance({
			...
			faviconUrl: String(settings.branding?.faviconUrl || '').trim(),
			supportEmail: String(settings.general?.supportEmail || '').trim(),
			contactEmail: String(settings.contact?.contactEmail || '').trim(),
			primaryDomain: String(settings.domains?.primaryDomain || '').trim(),
			appUrl: String(settings.domains?.appUrl || '').trim(),
			documentationUrl: String(settings.domains?.documentationUrl || '').trim(),
			canonicalUrl: String(settings.seo?.canonicalUrl || '').trim(),
			seo: {
				browserTitle: String(settings.seo?.browserTitle || '').trim(),
				metaTitle: String(settings.seo?.metaTitle || '').trim(),
				metaDescription: String(settings.seo?.metaDescription || '').trim(),
				metaKeywords: String(settings.seo?.metaKeywords || '').trim(),
				canonicalUrl: String(settings.seo?.canonicalUrl || '').trim(),
				ogTitle: String(settings.seo?.ogTitle || '').trim(),
				ogDescription: String(settings.seo?.ogDescription || '').trim(),
				ogImageUrl: String(settings.seo?.ogImageUrl || '').trim(),
				twitterCardType: settings.seo?.twitterCardType === 'summary' ? 'summary' : 'summary_large_image',
				twitterTitle: String(settings.seo?.twitterTitle || '').trim(),
				twitterDescription: String(settings.seo?.twitterDescription || '').trim(),
				twitterImageUrl: String(settings.seo?.twitterImageUrl || '').trim(),
				googleSiteVerification: String(settings.seo?.googleSiteVerification || '').trim(),
				bingSiteVerification: String(settings.seo?.bingSiteVerification || '').trim(),
				pinterestSiteVerification: String(settings.seo?.pinterestSiteVerification || '').trim(),
				facebookDomainVerification: String(settings.seo?.facebookDomainVerification || '').trim(),
			},
			defaultLanguage: settings.general?.defaultLanguage || 'en',
			timezone: settings.general?.timezone || 'UTC',
