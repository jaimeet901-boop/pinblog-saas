...
describe('gemini-models policy', () => {
	...
	it('marks retired Gemini ids unavailable', () => {
		...
		assert.equal(isRetiredGeminiModel('gemini-2.5-flash-001'), true);
		assert.equal(isRetiredGeminiModel('gemini-3.5-flash'), false);
		assert.equal(isRetiredGeminiModel('gemini-2.5-flash-image'), false);
		assert.equal(isRetiredGeminiModel('gemini-3-pro-image-preview'), false);
	});
	...
	it('exposes a non-retired stable image fallback', async () => {
		const { GEMINI_STABLE_IMAGE_FALLBACK_MODEL } = await import('./gemini-models.js');
		assert.ok(GEMINI_STABLE_IMAGE_FALLBACK_MODEL);
		assert.equal(isRetiredGeminiModel(GEMINI_STABLE_IMAGE_FALLBACK_MODEL), false);
	});
});
