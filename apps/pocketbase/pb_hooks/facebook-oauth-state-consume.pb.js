...
/**
 * F3: atomically consume a Facebook OAuth state.
 *
 ...
 * - BadRequestError()
 *
 * Superuser-only (Express pocketbaseClient). Does not call Facebook/Graph.
 * Does not open a second SQLite handle from Express.
 */

routerAdd("POST", "/api/facebook/oauth-states/consume", (e) => {
	const SAFE_MESSAGE = "Facebook connection could not be completed. Please try connecting again.";
	...
	try {
		result = $app.db()
			.newQuery(
				"UPDATE facebook_oauth_states SET used = true WHERE id = {:id} AND (used = false OR used = 0 OR used IS NULL) AND datetime(replace(replace(expires_at, 'T', ' '), 'Z', '')) > datetime('now')",
			)
			.bind({ id: id })
			.execute();
	} catch (_) {
